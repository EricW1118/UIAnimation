...
@implementation UIAnimationImage

+(UIAnimationImage*)actionWithImageArr:(NSArray*)imagesArr Duration:(CGFloat)duration
{
    UIAnimationImage* action = [[UIAnimationImage alloc] init];
    action.imageArray = imagesArr;
    action.interval = duration;
    [action setAnimationType:AnimationImage];
    return action;
}

+(UIAnimationImage*)actionWithImageArr:(NSString*)prex
                              ...
                              Duration:(CGFloat)duration
{
    ...
    {
        [imageArr addObject:[NSString stringWithFormat:@"%@%d.png",prex,i]];
    }
    return [self actionWithImageArr:imageArr Duration:duration];
}
